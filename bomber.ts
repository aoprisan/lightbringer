...
function start(): void {
  ...
  // A resting thumb trembles a few px; without slack that tremor keeps nudging
  // the nose and bleeds the straight-and-level run the bombsight demands.
  const STICK_DEAD = 8;
  ...
  svg.addEventListener("pointermove", (e) => {
    ...
    if (pointers.size === 1 && stick && stick.id === e.pointerId) {
      ...
      const mag = Math.hypot(dx, dy);
      // Dead zone, then re-scale so full deflection still sits at the rim.
  ...
  function gameUrl(): string {
    ...
    return native ? PUBLIC_SITE + "bomber.html" : location.origin + location.pathname;
  }
