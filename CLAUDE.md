...
- **Barricades are movement-only obstacles, not part of the light graph.** The `"barrier"` node kind (seeded by `generateCity` per the level's `barrierCount`) is a physical wall: `finalizeCity` **skips barriers entirely** — they get no edges and aren't neighbour candidates, so they hold no street and the turn-based spread is built *as if they were not there* (deliberately, per the design call: "movement blockers only"). They can't be kindled/awakened/decoyed, and `litStats`/`districtStats` exclude them from the lightable `total`. The only place they bite is the Lamplighter Run: `avoidBarriers` pushes the avatar (and only the avatar — Keepers float free) out of `BARRIER_RADIUS + PLAYER_RADIUS`. Because they ride the existing `kind` field, they need **no save bump**. Keep them out of the graph: any rule that walks `g.adj` already ignores them for free.
- **The city can be *won*, not only outlived.** Beside the carrier burning out (`maxFlame <= 0`, falling `BURN_PER_DAWN`/dawn), there is now a positive end: `cityWoke(g)` — `sustainedLit(g)/total >= CITY_WAKE_PCT` held by `>= CITY_WAKE_SOULS` awakened souls. `sustainedLit` reuses `applyDawn`'s flood (light soul-connected through other lit/awakened nodes), so the win is measured on *durable* light, not transient creep. It's checked **live each breath** in both shells (`breathe`, `actionFrame`) and on idle catch-up, and resolves through `winCity()`, which folds a `recordRun(g, true)` win into the legacy (`Legacy.wins`, its own key, no save bump). This does **not** soften the burnout invariant — both ends coexist; the eased economy (every city's `startFlame` raised, so the carrier gets more nights before burning out) is what makes the win reachable.

### Cities are levels — `generateCity` is parameterized, the rules are not

The one procedural map is now one of several hand-tuned **cities** (`LEVELS: LevelDef[]`). A `LevelDef` is a pure bundle of *generation + economy* overrides — node count, density (`minDist`), conduit fraction, press/shrine/keeper/**barrier** counts, keeper spacing, base keeper radius, start flame, weather temperament (`sky`), and the five quarter names. `generateCity(level)` and `freshGame(level)` read it; **no rule changes per city** (spread, Keepers, dawn, hearths, the carrier's burn are identical), so each city is a different *puzzle in the same language*, not a new game. `LEVELS[0]` ("The Old City") is the original generation kept exactly; the rest lean the same dials in distinct directions (e.g. Ashfold: dense + conductive + windy; The Drowned Quarter: sparse + rain-drowned + patient). Vesper Row is gated behind a legacy `bestNight` (`unlockNight`).
...
`saveGame`/`loadGame` use `localStorage` key `lightbringer.save.v5`. The save stores the city id (`level`) plus only per-node scalars (position, kind, state, brightness, revealed, heat, veil, patrol position, and `nights`) as compact arrays — **edges and adjacency are NOT saved**; `finalizeCity` rebuilds them deterministically from node positions/kinds on load, and the resolved `level` (an unknown id falls back to The Old City) supplies the quarters `makeNode` recomputes each node's district from. So edge/adjacency/district generation must stay a pure function of node geometry + the city's districts. Transient per-node fields like `decoy` are deliberately **not** saved. Barricades ride the existing `kind` scalar, so they round-trip with no shape change (`finalizeCity` rebuilds the graph around them on load). If you change the save shape, bump the version (`v`, `SAVE_KEY`, and the guard in `loadGame`) so old saves are rejected rather than mis-parsed.

The cross-run **legacy** (`loadLegacy`/`recordRun`, key `lightbringer.legacy.v1`) is kept in its **own** key, deliberately apart from the save: it survives "Begin again" (which only clears `SAVE_KEY`), carries between the classic and Lamplighter shells, and so **never forces a save-version bump**. It is a lifetime tally (runs ended, furthest night, brightest morning, most hearths, **cities woken**) folded in exactly once per run — at the three end transitions, `dawn()` (flame spent), `actionGameOver()` (caught in the Run), and `winCity()` (the city woke; `recordRun(g, true)`) — and read (not written) on the intro/end overlays via `legacyHtml`. Keep it write-once-per-end: don't record it on the reload-into-end branches, or runs double-count.
