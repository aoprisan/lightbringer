...
node tools/gen-eld-art.mjs       # regenerate the eldritch place establishing scenes (art/place-*.png)
...
`spriteFor`/`loadCitySprites` (`art/<cityId>/<name>.png`, silent fallback to the base). **Every city now
carries an `art` establishing scene** shown on the picker card: the first five are painted jpgs
(`art/city-*.jpg`), and the Foundry, the Bastion and the four Edge-Lands cities are **generated** zero-dep
by `tools/gen-pg-art.mjs` (`art/city-*.png`) — all eleven are cache-first in `sw.js` `ASSETS`.
...
The villages are now a **full campaign** (a mirror of the werewolf's): each `LevelDef` carries an
`epigraph` **and a `story` chapter** linking the village before to the village after, a `PROLOGUE` frames
the very first march and an `EPILOGUE` plays once every village is overrun, and the villages **unlock in
LEVELS order** (`villageUnlocked` — each opens once the one before it has a `best` overrun; the picker shows
...
(`art/place-<id>.png`, generated by `tools/gen-eld-art.mjs`); a `PROLOGUE` frames the very first watch and
an `EPILOGUE` plays once every threshold is sealed. The places **unlock in LEVELS order** (`placeUnlocked` —
each opens once the one before it has a `best` seal; the picker shows locked ones veiled, and the win screen
names the next place or plays the epilogue). A duel link deliberately bypasses the gate (a guest pass).
...
`recordFall`. Every sprite has a **procedural SVG fallback** (`scenerySprite`, `pentagramPath`, the render
fallbacks), so the game is **fully playable with zero gameplay PNGs** — its in-arena sprites still ship
that way (**no gameplay sprite PNGs yet**; the Watcher/host/ward render from vector primitives — when those
sprites ship, add them to `ASSETS` **and** bump `CACHE`). What DOES ship generated: the four **campaign
establishing scenes** (`art/place-*.png`, `tools/gen-eld-art.mjs`, shown on the picker card, cache-first in
`sw.js` `ASSETS`) and its **PWA icons** — a glowing Elder Sign over the threshold generated zero-dep by
`tools/gen-eld-icons.mjs` (`icons/eldritch-icon-{192,512,180}.png` + maskable), listed in `sw.js` `ASSETS`.
Sprite resolution mirrors the siblings (`spriteFor`/`loadSprites`/`loadCitySprites`).
...
The theatres are now a **full campaign** (a mirror of the werewolf's): each `LevelDef` carries an
`epigraph` **and a `story` chapter** linking the raid before to the raid after, plus an `art` establishing
scene (`art/theatre-<id>.png`, generated by `tools/gen-bomber-art.mjs`); a `PROLOGUE` frames the very first
raid and an `EPILOGUE` plays once every theatre is silenced. The theatres **unlock in LEVELS order**
(`theatreUnlocked` — each opens once the one before it has a `best` raid; the picker shows locked ones
veiled, and the win screen names the next theatre or plays the epilogue). A duel link deliberately bypasses
the gate (a guest pass).
...
per-kind work marks), so the game is **fully playable with zero gameplay PNGs** — its in-arena sprites
still ship that way (**no gameplay sprite PNGs yet**; when the bomber/fighter/works sprites ship, add them
to `ASSETS` **and** bump `CACHE`). What DOES ship generated: the four **campaign establishing scenes**
(`art/theatre-*.png`, `tools/gen-bomber-art.mjs`, shown on the picker card, cache-first in `sw.js`
`ASSETS`) and its **PWA icons** — a bomber silhouette caught in crossing searchlights, generated zero-dep by
`tools/gen-bomber-icons.mjs` (`icons/bomber-icon-{192,512,180}.png` + maskable), listed in `sw.js` `ASSETS`.
Sprite resolution mirrors the siblings (`spriteFor`/`loadSprites`/`loadCitySprites`).
