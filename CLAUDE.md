...
repository. There are **four** games (the first three share one world — a city taught that *light burns*;
the fourth is a Lovecraftian sibling):
...
`app.ts` → `app.js`, `pentagram.ts` → `pentagram.js`, `necro.ts` → `necro.js`, and `eldritch.ts` →
`eldritch.js`; those `.js` files are what GitHub Pages serves. All four games **cross-link** from each
other's title/header.
...
tools/necro-test.mjs && node tools/eldritch-test.mjs` — compile, then all four suites in sequence.

The `.js` files (`app.js`, `pentagram.js`, `necro.js`, `eldritch.js`) are **build artifacts** — git-ignored, regenerated by
...
"eldritch.ts"]`; `lightbringer.ts` is excluded (reference-only prototype).
...
`<script type="module">`. This is required: all four are in `tsconfig.json`'s `include`, and scriptless
files would collide on every top-level name (`W`, `el`, `render`, `start`, …). Module scope keeps the three
...
`sw.js` is the offline app-shell cache for **all four games**, with an explicit `ASSETS` list and a `CACHE`
version string (currently `lightbringer-v94`). It is **network-first for the shells** (`isShell`: `/`,
`index.html`, `app.js`, `pentagram.html`, `pentagram.js`, `necro.html`, `necro.js`, `eldritch.html`,
`eldritch.js`) so the freshest code always wins online, and **cache-first** for the heavy, slow-changing
