...
### Persistence (save format v4)

`saveGame`/`loadGame` use `localStorage` key `lightbringer.save.v4`. Only per-node scalars are stored (position, kind, state, brightness, revealed, heat, veil, patrol position, and `nights`) as compact arrays — **edges and adjacency are NOT saved**; `finalizeCity` rebuilds them deterministically from node positions/kinds on load. So edge/adjacency generation must stay a pure function of node geometry. Transient per-node fields like `decoy` are deliberately **not** saved. If you change the save shape, bump the version (`v`, `SAVE_KEY`, and the guard in `loadGame`) so old saves are rejected rather than mis-parsed.
