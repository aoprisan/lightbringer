...
Four terrain mechanics dress the descent on top of the kill-the-host core:
...
- **Frescoes (the carrier's first-footing).** The same painted fragments as the parent, but the trigger is the hero's *body* reaching a place, not light spreading: in `stepCombat`, a scenery node first brought within `FRESCO_REACH` of the hero is marked `node.seen` and rolls `maybeFresco` (own `FRESCOES`/`FRESCO_ART` pool — modules don't share `app.ts`'s — presses/shrines always carry, plainer ground at 0.06). Pure sim, mirroring `app.ts`: it only queues `s.pendingFresco`. The shell (`pgFrame`) shows it **without pausing** — a small non-interactive corner card (`#fresco`, `showFresco`) that fades in, lingers, and fades on its own timer (an early version froze the descent, which killed the flow). Painted art is **drop-in**: `FRESCO_ART` pre-wires all seventeen `art/fresco-*.jpg`, but `showFresco` only reveals the `<img>` once it actually loads, so a line whose art is not generated yet shows as text alone (same probe-then-show fallback in `app.ts`'s `revealFresco`). Only the original five fresco jpgs are in `sw.js` ASSETS; the rest are opportunistically cached online — when you generate one (`art-prompts/06f`–`06q`), add it to ASSETS and bump `CACHE` for offline. One fresco at a time, each shown once (`s.shownFrescoes`), finite pool. Like decoys/fences, frescoes are **live-play state, not persisted**.
